...
Usage:
    python -m fluvio_planner.evals.harness --prompt step_formulation
...
if __name__ == "__main__":
    import os
    import sys
    # The eval CLI is a self-contained composition root: it reads the key
    # straight from the environment rather than importing the service's Settings,
    # so this package has no dependency back on services/agent-planner.
    name = sys.argv[1] if len(sys.argv) > 1 else "step_formulation"
    result = asyncio.run(run_eval(name, os.environ.get("ANTHROPIC_API_KEY")))
