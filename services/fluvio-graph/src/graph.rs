...
use fluvio_types::{
    Domain, GraphError, GraphQuery, GraphEvent, GraphResult, NodeKind,
    NodeId, EdgeId, GraphId, Node, Edge, DomainGraph,
};
...
impl FluvioGraph for DomainGraph {
    ...
    fn query(&self, query: GraphQuery) -> GraphResult {
        match query {
            ...
            GraphQuery::ShortestPath { from, to } => {
                GraphResult::Path(self.shortest_path(from, to))
            }
            GraphQuery::Filter(pred) => {
                let nodes = self.nodes.values()
                    .filter(|n| DomainGraph::matches_predicate(n, &pred))
                    .cloned()
                    .collect();
    ...
    fn subscribe(&self, _event: GraphEvent) -> broadcast::Receiver<GraphEvent> {
        self.tx.subscribe()
    }
