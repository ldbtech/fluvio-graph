...
import { getToken, getTwinUserId } from "@/lib/fluvioDashboardApi";
import type { NfcCardDesign, NfcCardThemeId, WifiPreorderShipping } from "@/lib/hardwareOrders";
import { DEFAULT_NFC_ACCENT_HEX, DEFAULT_NFC_THEME_ID } from "@/lib/hardwareOrders";
import { WIFI_NFC_PREORDER_ENABLED } from "@/lib/onboardingFlags";
import { WIFI_PREORDER_SHIPPING_EMPTY, placeNfcCardOrder, placeWifiNfcPreorder } from "@/lib/hardwareOrders";
...
export type { NfcCardDesign, NfcCardThemeId, WifiPreorderShipping } from "@/lib/hardwareOrders";
...
const CARD_THEMES: Record<NfcCardThemeId, CardThemeStyle> = {
  carbon: {
    label: "Carbon",
...
const NFC_THEME_IDS_ORDERED = Object.keys(CARD_THEMES) as NfcCardThemeId[];

function sanitizeAccent(hex: string): string {
  return /^#[0-9A-Fa-f]{6}$/.test(hex) ? hex : DEFAULT_NFC_ACCENT_HEX;
}

function NfcCardFrontPreview({ design }: { design: NfcCardDesign }) {
