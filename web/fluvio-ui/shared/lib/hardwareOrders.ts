...
function parseNfcCardDesignFields(d: Record<string, unknown>): NfcCardDesign {
  return {
...
/** Parse a loose object (e.g. `nfcDesign` from onboarding localStorage). */
export function parseLooseNfcCardDesign(raw: unknown): NfcCardDesign | null {
  if (!isRecord(raw)) return null;
  return parseNfcCardDesignFields(raw);
}

/**
 * Latest saved tap-card design for this browser session: most recent NFC card order,
 * else `nfcDesign` from completed onboarding (`fluvio_onboarding_v2`).
 * Keep storage key in sync with OnboardingClient.
 */
const FLUVIO_ONBOARDING_STORAGE_KEY = "fluvio_onboarding_v2";

export function resolveSessionNfcCardDesign(sessionOwnerId: string | null): NfcCardDesign | null {
  const orders = hardwareOrdersForSession(listHardwareOrders(), sessionOwnerId);
  const nfc = orders.find((o): o is NfcCardOrder => o.kind === "nfc_card");
  if (nfc) return { ...nfc.design };

  if (typeof window === "undefined") return null;
  try {
    const parsed = JSON.parse(localStorage.getItem(FLUVIO_ONBOARDING_STORAGE_KEY) ?? "null");
    if (!isRecord(parsed) || parsed.nfcDesign === undefined || parsed.nfcDesign === null) return null;
    return parseLooseNfcCardDesign(parsed.nfcDesign);
  } catch {
    return null;
  }
}
...
function parseOrder(raw: unknown): HardwareOrder | null {
  ...
  if (kind === "nfc_card" && isRecord(raw.design)) {
    const design = parseNfcCardDesignFields(raw.design);
    const shipping = "shipping" in raw ? parseShipping(raw.shipping) : { ...WIFI_PREORDER_SHIPPING_EMPTY };
    return {
      id,
      kind: "nfc_card",
