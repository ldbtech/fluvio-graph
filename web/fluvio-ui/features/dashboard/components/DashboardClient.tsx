...
/** Optional dev helper: second user’s card UUID → show as a ghost node until you connect for real. */
const DEV_PREVIEW_PEER_CARD =
  typeof process !== "undefined" && process.env.NEXT_PUBLIC_FLUVIO_DEV_PEER_CARD_ID
    ? process.env.NEXT_PUBLIC_FLUVIO_DEV_PEER_CARD_ID.trim()
    : "";

const PREVIEW_PEER_NODE_ID = "__fluvio_preview_peer_card__";
...
export function DashboardClient() {
  ...
  const [peerCardInput, setPeerCardInput] = useState("");
  ...
  const circleGraph = useMemo(() => {
    const nodes = [...socialNodes];
    const edges = [...socialEdges];
    const hasRealConnections = socialNodes.length > 1;
    if (
      DEV_PREVIEW_PEER_CARD.length >= 32 &&
      ownerGraphCenterId &&
      !hasRealConnections
    ) {
      const hint = DEV_PREVIEW_PEER_CARD.slice(0, 8);
      nodes.push({
        id:     PREVIEW_PEER_NODE_ID,
        label:  `Peer card …${hint}`,
        page:   "dev",
        source: "preview",
      });
      edges.push({
        from:        ownerGraphCenterId,
        to:          PREVIEW_PEER_NODE_ID,
        token:       40,
        probability: 0.45,
        label:       "sim tap",
      });
    ...
    return { nodes, edges };
  }, [socialNodes, socialEdges, ownerGraphCenterId]);
  ...
  return (
    <div className="min-h-[100dvh] bg-zinc-950 text-white antialiased">
      ...
      {authStatus === "signed_in" ? (
        <div className="border-b border-white/[0.06] bg-zinc-950/95">
          <AuthedProfileHeader />
        </div>
      ) : null}

      <main
        className={`mx-auto max-w-5xl px-5 pb-[max(2.5rem,env(safe-area-inset-bottom))] sm:px-8 ${
          authStatus === "signed_out"
            ? "flex min-h-[calc(100dvh-6.75rem)] items-center justify-center pt-6 sm:min-h-[calc(100dvh-7.5rem)] sm:pt-8"
            : "space-y-10 pt-10"
        }`}
      >
        {authStatus === "signed_in" ? <DashboardHardwareOrders sessionOwnerId={sessionOwnerId} /> : null}
        ...
        {authStatus === "loading" ? (
          ...
          <section className="relative mx-auto w-full max-w-[560px] overflow-hidden rounded-[28px] border border-white/[0.08] bg-[linear-gradient(160deg,rgba(255,255,255,0.06)_0%,rgba(255,255,255,0.02)_40%,rgba(255,255,255,0.01)_100%)] p-5 shadow-[0_24px_80px_-40px_rgba(0,0,0,0.85)] sm:p-10">
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(255,255,255,0.08),transparent_55%)]" aria-hidden />
            <div className="relative mx-auto w-full max-w-md text-center">
              <h2 className="text-[1.9rem] font-semibold tracking-[-0.04em] text-white sm:text-[2.2rem]">Sign in</h2>
              <p className="mt-3 text-pretty text-[15px] leading-relaxed text-zinc-400 sm:text-[16px]">
                One-time code via email. If this is your first time, we create your account right after verification.
              </p>
              <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-white/[0.12] bg-black/30 p-1">
                <span
                  className={`rounded-full px-3 py-1.5 text-[11px] font-medium tracking-wide ${
                    loginStep === "email" ? "bg-white text-zinc-950" : "text-zinc-500"
                  }`}
                >
                  1. Email
                </span>
                <span
                  className={`rounded-full px-3 py-1.5 text-[11px] font-medium tracking-wide ${
                    loginStep === "code" ? "bg-white text-zinc-950" : "text-zinc-500"
                  }`}
                >
                  2. Code
                </span>
            ...
            <div className="mt-8 grid gap-4 text-left">
              <label className="block">
                ...
                <input
                  ...
                  className="mt-1.5 w-full rounded-2xl border border-white/[0.12] bg-zinc-950/80 px-4 py-3.5 text-[16px] text-white placeholder:text-zinc-600 transition focus:outline-none focus:ring-2 focus:ring-violet-500/30 disabled:opacity-50"
                />
              </label>
              {loginStep === "email" ? (
                <label className="block">
                  ...
                  <input
                    ...
                    className="mt-1.5 w-full rounded-2xl border border-white/[0.12] bg-zinc-950/80 px-4 py-3.5 text-[16px] text-white placeholder:text-zinc-600 transition focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                  />
                </label>
              ) : null}
              {loginStep === "code" ? (
              ...
              {demoCodeHint ? (
                <p className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.08] px-4 py-3 text-[14px] text-amber-200/95" role="status">
                  Demo mode — your code: <span className="font-mono font-semibold">{demoCodeHint}</span>
                </p>
              ) : null}
              {loginErr ? (
                <p className="rounded-2xl border border-red-500/25 bg-red-500/[0.08] px-4 py-3 text-[14px] text-red-300/95" role="alert">
                  {loginErr}
                </p>
              ) : null}
              {loginStep === "email" ? (
                <button
                  ...
                  className="mt-1 w-full rounded-full bg-white px-5 py-3.5 text-[16px] font-semibold text-zinc-950 transition hover:bg-zinc-100 disabled:opacity-40"
                >
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="w-full rounded-full bg-white px-5 py-3.5 text-[16px] font-semibold text-zinc-950 transition hover:bg-zinc-100 disabled:opacity-40 sm:w-auto sm:flex-1"
                  >
                  ...
                  <button
                    ...
                    className="w-full rounded-full border border-white/[0.14] bg-white/[0.02] px-5 py-3.5 text-[16px] font-medium text-zinc-300 transition hover:bg-white/[0.07] sm:w-auto"
                  >
                    Edit email
                  </button>
                </div>
            ...
            <p className="mt-8 text-[14px] text-zinc-600 sm:text-[15px]">
        ...
        {authStatus === "signed_in" && account ? (
          <DashboardAppleWallet
            ...
            nfcCardId={account.nfc_card_id ?? null}
          />
        ) : authStatus === "signed_in" && !loadErr ? (
          <p className="text-[15px] text-zinc-500">Loading…</p>
        ) : null}
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          >
            ...
            <p className="mt-2 max-w-sm text-[15px] leading-relaxed text-zinc-500">
              Notes you save here attach to your tap profile for your twin.
            </p>
          ...
          <section className="flex min-h-[280px] flex-col rounded-[20px] border border-white/[0.06] bg-white/[0.02] p-6 sm:min-h-[320px] sm:p-8">
            ...
            <p className="mt-2 text-[15px] text-zinc-500">
              Tap someone below to see more, or open My Network for the full view. Your NFC card id is created automatically so you can test taps without ordering hardware.
            </p>

            {account?.nfc_card_id ? (
              <div className="mt-5 rounded-2xl border border-white/[0.06] bg-zinc-950/50 px-4 py-4">
                <p className="text-[13px] font-medium text-zinc-400">Your NFC card ID</p>
                <p className="mt-1.5 break-all font-mono text-[13px] leading-relaxed text-zinc-200">
                  {account.nfc_card_id}
                </p>
                <p className="mt-3 text-[12px] leading-relaxed text-zinc-600">
                  Simulate tap: paste another user&apos;s card UUID below while signed in as{" "}
                  <span className="text-zinc-500">you</span> — that creates the connection in Postgres so
                  chat can use <span className="font-mono text-[11px] text-zinc-500">graph_owner_id</span>.
                  Or open{" "}
                  <span className="break-all font-mono text-[11px] text-violet-400/90">
                    {getKgEngineUrl()}/twin/tap/{account.nfc_card_id}
                  </span>{" "}
                  in another profile to connect to you.
                </p>
                <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
                  <label className="block min-w-0 flex-1">
                    <span className="text-[12px] text-zinc-500">Other person&apos;s card UUID</span>
                    <input
                      type="text"
                      value={peerCardInput}
                      onChange={(e) => setPeerCardInput(e.target.value)}
                      placeholder="00000000-0000-0000-0000-000000000000"
                      className="mt-1.5 w-full rounded-xl border border-white/[0.08] bg-zinc-950 px-3 py-2.5 font-mono text-[13px] text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                    />
                  </label>
                  <button
                    type="button"
                    disabled={tapBusy || !peerCardInput.trim()}
                    onClick={() => {
                      void (async () => {
                        setTapBusy(true);
                        setLoadErr(null);
                        try {
                          await tapCard(peerCardInput.trim());
                          setPeerCardInput("");
                          await reload();
                        } catch (e) {
                          setLoadErr(e instanceof Error ? e.message : "Simulate tap failed");
                        } finally {
                          setTapBusy(false);
                        }
                      })();
                    }}
                    className="shrink-0 rounded-full border border-white/[0.12] bg-white/[0.06] px-5 py-2.5 text-[15px] font-medium text-white transition hover:bg-white/[0.1] disabled:opacity-40"
                  >
                    {tapBusy ? "Connecting…" : "Simulate tap"}
                  </button>
                </div>
                {DEV_PREVIEW_PEER_CARD ? (
                  <p className="mt-3 text-[12px] leading-relaxed text-zinc-600">
                    Dev: <code className="text-zinc-400">NEXT_PUBLIC_FLUVIO_DEV_PEER_CARD_ID</code> adds a
                    placeholder peer on the map until you have a real connection.
                  </p>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="mt-4 min-h-[220px] flex-1 overflow-hidden rounded-2xl ring-1 ring-white/[0.06] sm:min-h-[260px]">
              {circleGraph.nodes.length > 0 ? (
                <DashboardForceGraph
                  nodes={circleGraph.nodes}
                  edges={circleGraph.edges}
                  centerId={ownerGraphCenterId || undefined}
                  onNodeClick={(id) => {
                    if (id === PREVIEW_PEER_NODE_ID) return;
            ...
            {selectedConn ? (
              <div className="mt-8 border-t border-white/[0.06] pt-8">
                ...
                <div className="mt-5 rounded-2xl border border-white/[0.08] bg-zinc-950/60 p-5 sm:p-6">
                  ...
                  <Link
                    href={`/graph?topic=${encodeURIComponent(selectedConnection?.name ?? "")}`}
