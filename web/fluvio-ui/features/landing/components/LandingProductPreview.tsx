...
function CreateFluvioMePreview() {
  return (
...
function WorkspacePreviewSilhouette() {
  return (
...
function ChatPreviewSilhouette() {
  return (
...
function DashboardPreviewSilhouette() {
  return (
...
export function LandingProductPreview() {
  const tiles: Tile[] = [
    ...
    },
    {
      href: "/dashboard",
      title: "Overview",
      subtitle: "Ingest and manage memory",
      span: "md:col-span-6 md:col-start-7 md:row-start-3",
      preview: (
        <>
          <WindowDots />
          <WorkspacePreviewSilhouette />
        </>
      ),
    },
  ];
        ...
        <div className="mt-14 grid grid-cols-1 gap-4 sm:gap-4 md:grid-cols-12 md:grid-rows-3 md:auto-rows-min md:gap-4">
          {tiles.map((t) => (
            ...
            >
              ...
              <div className="flex min-h-0 flex-1 flex-col bg-black/20">{t.preview}</div>
            </Link>
          ))}
        </div>
      </div>
